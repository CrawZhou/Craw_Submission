...
// Core filter function: check if a frame contains both car and truck
func hasBothCarAndTruck(frameDetections []Detection) bool {
	hasCar := false
	hasTruck := false

	// Iterate through all detections, check if both car and truck exist
	for _, det := range frameDetections {
		...
		// Early exit: when both car and truck are found, stop iterating
		if hasCar && hasTruck {
			return true
		}
	}

	// Need both car and truck to return true
	return hasCar && hasTruck
}
...
func main() {
	...
	for i := 0; i < iterations; i++ {
		...
		// Filter frames with both car and truck (call the new filter function)
		frameCount := 0
		for _, frameDetections := range baselineDetections {
			if hasBothCarAndTruck(frameDetections) { // Replace original hasMoreThan4Cars function
				frameCount++
			}
		}
		...
		// Print iteration progress, text updated to "frames with both car and truck"
		fmt.Printf("Iteration %d/%d completed, time: %.3f ms, found matching frames (car and truck): %d\n",
	...
	// Output final statistics, text updated to "frames with both car and truck"
	fmt.Println("\n===== Final Statistics =====")
	fmt.Printf("Total queries executed: %d\n", iterations)
	fmt.Printf("Average query time: %.3f ms\n", averageTimeMs)
	fmt.Printf("Average matching frames per query (car and truck): %d frames\n", averageFrameCount)
